...
a  =  list()
a[[1]] = (1:10)/10#sort(runif(K_grid[1]))
...
{ 
  ...
  for(ind_n in 1:length(n_grid))
  {
    ...
    for(iter  in 1:NMC)
    {
      ...
      ######################
      A_name  =  paste("A_ind4",'n',toString(n),'K',toString(K),"iter",iter,sep='')
      A_name = paste(output_directory,A_name,".txt",sep="")
      write.table(A,A_name,col.names = FALSE,row.names = FALSE)
      
      P_name  =  paste("P_ind4",'n',toString(n),'K',toString(K),"iter",iter,sep='')
